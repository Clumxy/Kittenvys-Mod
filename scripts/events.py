...
class Events():
            ...
            for other_clan in game.clan.all_clans:
                if int(other_clan.relations) <= 5:
                    if randint(1, 5) == 1 and self.time_at_war > 2:
                        ...
                        f'{game.clan.name}Clan start sending more patrols near the {other_clan.name}Clan border.'
                        ]
        ...
        if randint(1, chance) == 1 and cat.age != 'kitten' and cat.age != 'adolescent' and not self.new_cat_invited:
            ...
            if type_of_new_cat == 1:
                ...
                loner_text = [
                    ...
                    f'An unknown cat named {loner_name.prefix} stumbled into your camp. They were immediately confronted and through nervous slurred words explained that they didnt intend to fall in. They were just interested in clan life, and the rogue always wanted to be a warrior one day. They join the clan eventually.'
                ]
                ...
                if len(game.clan.all_clans) > 0:
                    warrior_text.extend([
                        ...
                        f'a outsider from a other clan named {warrior_name} asks the leader to join the clan to be with {name}.'
                    ])
                else:
                    warrior_text.extend([
    ...
    def other_interactions(self, cat):
        ...
        if cat.status == 'kitten' and other_cat.status != 'kitten':
            interactions.extend([
                ...
                f'{name} and {other_name} play a game of mossball!',
            ...
            interactions.extend([
                ...
                f'{name} is spotted on the clan border, seeming to be talking to a warrior from a other Clan.',
                f'{name} seems to be distracted from clan duties.',
                f'{name} is plotting something...',
                f'{name} was visited by Starclan and was told that they had a bright future ahead of them',
                f'A patrol has scented a fox lingering on the border',
                f'{name} seems to be acting suspicious.',
                f'{name} warns the clan about something, but the cats cant identify whats going to happen.',
                f'The clan finds a new type of prey.',
                f'The clan finds a new type of herb.',
                f'The clan drives away a pack of dogs sucessfully.',
                f'{name}s plot to kill the leader is discovered and the cat is punished.',
                f'A hawk terrorizing the Clan is brought down and feeds the entire Clan',
                f'A disease spreaded throughout the camp but doesnt affect the cats. Alternatively, it will only murder the prey that they eat. Now, theres plenty of deceased prey around your territory which will be hard to clean up. This attracts a lot of predators, your rival clan, and rogues but is a very nice meal for your cats.',
                f'The {otherclan}Clan Leader has recently lost their last life.',
                f'{name} has a hunting competition with {other_name} and loses.',
                f'{otherclan}Clan attempted to steal a small section of territory, but they lost the fight and had to retreat.',
                f'{name} gets praised at the gathering.'
                f'{name} is accused of stealing prey, but they are innocent.',
             ])
            if cat.status == 'warrior' and other_cat.status == 'warrior':
             interactions.extend([
                f'{name} thinks that {other_name} is a traitor',
                f'{name} reveals that {other_name} is a traitor',
                f'{name} reports that they saw {other_name} talking to a loner.',
                f'{name} challenges {other_name} to see whos better!',
                f'{name} Is worried about {other_name}',
                f'{name} and {other_name} have a fight with eachother in the middle of the camp.',
                f'{name} helps {other_name} with a problem their having.',
                f'{name} notices {other_name} being weird, and then realizes a shocking truth.',
                f'{name} saves {other_name}.'
                ])
            if cat.status == 'warrior' and other_cat.status == 'deputy':
             interactions.extend([
                f'{name} competes with {other_name} to see who can be a better deputy',
                f'{name} doesnt like the sound of {other_name} being deputy',
                f'{name} congratulates {other_name} for their new role.'
                ])
            ...
            interactions.extend([
                ...
                f'{name} seems to be performing poorly, and seems unfocused.',
                f'{name} Refused to meet with the other medicine cats this moon.',
            ])
        elif cat.status == 'deputy':
            interactions.extend([
                f'{name} thinks about retiring.',
                f'{name} travels to the other clans to bring them an important message.',
                f'{name} has bad thoughts of murdering the leader.',
                f'{name} tries to get their reputation better.',
                f'{name} forms a coup against the leader.',
                f'{name} thinks about forming a coup.'
            ])
            ...
            interactions.extend([
                ...
                f'{name} visits StarClan at the Moonstone.'
            ])
        ...
        if not int(random.random() * 100) and cat.status == 'leader':  # 1/100
            ...
            if game.clan.all_clans:
                cause_of_death.extend([
                    ...
                    f'{choice(game.clan.all_clans).name}Clan breaks into camp, and {name} loses a life in the battle.'
                ])
            game.clan.leader_lives -= 1
            cat.die()
        ...
        if triggered_death and not int(random.random() * 200) and cat.status == 'leader':  # 1/200
            if not int(random.random() * 10): # 1/10
                ...
                if self.at_war and game.clan.all_clans:
                    cause_of_death.extend([
                        ...
                        f'{choice(game.clan.all_clans).name}Clan breaks into camp, and {name} loses all of their lives.'
                    ])
                if game.clan.biome == "Mountainous":
                    cause_of_death.extend([
                        f'{name} was buried alive in an avalanche',
                        f'{name} was buried alive by a landslide',
                        f'{name} was pushed off a cliff with sharp rocks at the botmale',
                        f'{name} accidentally fell off a cliff with sharp rocks at the botmale'
                    ])
            ...
            if cat.status == 'kitten':
                cause_of_death.extend([
                    ...
                    f'{name} sneaks out of camp and is found dead near a fox den',
                    f'{name} died after a cruel shadow killed them in the middle of the night'
                ])
                if game.clan.current_season == 'Leaf-bare':
                    cause_of_death.extend([
                ...
                cause_of_death.extend([
                    ...
                    f'{name} was bitten by an adder',
                    f'{name} hit their head on a rock and died',
                    f'{name} was impaled by a crystal'
                     ])
            elif cat.status == 'warrior' or cat.status == 'deputy':
                cause_of_death.extend([
                    f'{name} went missing and was found dead',
                    f'{name} is found by a patrol after they were missing for several days. Unfortunately they were found in a ditch covered by undergrowth, dead for several days. What killed them is a mystery.',
                    f'{name} was bitten by an adder',
                    f'{name} hit their head on a rock and died',
                    f'{name} was impaled by a crystal',
                    f'{name} was struck by lightning.',
                    f'{name} is mauled by a pack of dogs.',
                    f'{name} is killed after snapping their neck during a fight with a enemy warrior.',
                    f'{name} lost a limb and died',
                    f'{name} died saving the leader',
                    f'{name} gets mauled by dogs after saving a clanmate',
                    f'{name} pads thru the forest. As they hear a distant barking, they can identify the barking of a lot of dogs. The noise seems to be getting closer, and they notice {other_name} getting close to the dogs. The dogs notice them and attack. Before the dogs can attack {other_name}, {name} steps right in front of them and takes the blow. Once the battle is done, {other_name} realizes that {name} will never awaken',
                    f'{name} drank contaminated water and died',
                    f'{name} drowned trying to save {other_name}',
                    f'{name} ate crowfood out of desperation'
                ])
                if game.clan.all_clans:
    ...
    def handle_disasters(self,cat):
        ...
        if alive_count > 15:
            ...
            disaster.extend([
                    ...
                    ' stumble upon a badger den. The badger spots them and they are all never seen again.',
                    'die after a tornado hits the camp.'                    
                ])
            if game.clan.current_season == 'Leaf-bare':
                disaster.extend([
    ...
    def handle_illnesses_or_illness_deaths(self, cat):
        ...
        if not int(random.random() * 100) and cat.status == 'leader': # 1/100
            ...
            triggered_death = True
            lostlives = choice([2, 2, 2, 3, 3, 3, 4, 4, 4, 5, 5, 6, 7])
    ...
    def handle_deaths_expanded(self, cat, other_cat):
        ...
        if cat.status == 'kitten':
            cause_of_death.extend([
                ...
                f'{name} died after accidentally eating deathberries',
                f'{name} was killed in their sleep after a snake snuck into camp',
                f'{name} slips away from camp when no one is watching to explore! Unfortunately they wound up finding a den of fox cubs, and a patrol finds what is left of the poor kit the next day.'
            ])
            if game.clan.current_season == 'Leaf-bare':
                cause_of_death.extend([
                    f'{name} was found dead in the snow',
